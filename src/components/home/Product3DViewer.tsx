...
// Lazy load the WebGL viewer to avoid SSR issues
const Product3DViewerGL = lazy(() => import("./Product3DViewerGL"));

// Frame-based fallback images
const FRAME_IMAGES = [
...
// Frame-based viewer (fallback)
function FrameBasedViewer({ className = "" }: { className?: string }) {
  ...
  const handleMove = useCallback(
    (clientX: number) => {
      ...
      if (Math.abs(deltaX) > sensitivity) {
        ...
        setCurrentFrame(
          (prev) => (prev + direction + FRAME_IMAGES.length) % FRAME_IMAGES.length
        );
        lastXRef.current = clientX;
      }
    },
...
// Main component that tries GLB first, falls back to frames
export default function Product3DViewer({ className = "" }: Product3DViewerProps) {
  ...
  useEffect(() => {
    fetch("/models/bottom-feeder.glb", { method: "HEAD" })
      .then((response) => {
        setUseWebGL(response.ok);
        setChecking(false);
      })
      .catch(() => {
        setUseWebGL(false);
  ...
  if (useWebGL) {
    return (
      ...
      >
        <Product3DViewerGL className={className} />
      </Suspense>
    );
  }

  return <FrameBasedViewer className={className} />;
}
